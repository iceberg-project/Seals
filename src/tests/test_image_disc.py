...
@mock.patch('src.entk_script.image_disc.glob', return_value=['test1.tif','test2.tif'])
def test_image_discovery_1(mocked_glob):

    expected_values = pd.DataFrame(columns=['Filename'],data=['test1.tif','test2.tif'])
...
@mock.patch('src.entk_script.image_disc.glob', return_value=['test1.tif','test2.tif'])
@mock.patch('src.entk_script.image_disc.os.path.getsize', side_effect=[1048576,4194304])
def test_image_discovery_3(mocked_glob,mocked_getsize):

    expected_values = pd.DataFrame(columns=['Filename', 'Size'], data=[['test1.tif',1],['test2.tif',4]])
    # Test just the CSV file contents
