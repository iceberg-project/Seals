...
Based on https://github.com/rwightman/pytorch-countception-sealion/blob/master/models/model_countception.py
"""
import torch
    ...
    def __init__(self, inplanes=3, outplanes=1, use_logits=False, logits_per_output=12, debug=False):
        ...
        if use_logits:
            self.conv6 = nn.ModuleList([ConvBlock(
                32, logits_per_output, ksize=1, activation=self.final_activation) for _ in range(outplanes)])
        else:
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.ConvTranspose2d):
                nn.init.xavier_uniform_(m.weight, gain=init.calculate_gain('leaky_relu', param=0.01))
