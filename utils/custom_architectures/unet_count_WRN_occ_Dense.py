...
class UNetCntWRNOccDense(nn.Module):
    def __init__(self, depth, scale=64, n_channels=1, n_classes=1, widen_factor=1, drop_rate=0.5,
                 growth_rate=32, block_config=(6, 12, 24, 16), num_init_features=64, bn_size=4):
        super(UNetCntWRNOccDense, self).__init__()
        
        # WRN part for occupancy and counting
        ...
        self.n_channels = nChannels[3]
        self.conv1 = nn.Conv2d(scale*8, nChannels[0], kernel_size=3, stride=1,
                               padding=1, bias=False)
        # 1st block
        ...
        self.final_conv = nn.Conv2d(in_channels=nChannels[3], out_channels=1, kernel_size=1)

        # occupancy
        # First convolution
        self.features = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv2d(64, num_init_features, kernel_size=7, stride=2, padding=3, bias=False)),
            ('norm0', nn.BatchNorm2d(num_init_features)),
            ('relu0', nn.ReLU(inplace=True)),
            ('pool0', nn.MaxPool2d(kernel_size=3, stride=2, padding=1)),
        ]))
        ...
        # Linear layer
        self.fc_occ = nn.Linear(num_features, 1)
    ...
    def forward(self, x):
        ...
        # occupancy branch
        occ = self.features(x2)
        feature_size = occ.size(0)
        occ = self.relu(occ)
        occ = F.avg_pool2d(occ, kernel_size=3, stride=1).view(feature_size, -1)
        occ = self.fc_occ(occ)
        ...
        cnt = self.block2(cnt)
        cnt = self.block3(cnt)
        cnt = self.relu(self.bn1(cnt))
        cnt = self.final_conv(cnt)
        cnt = F.avg_pool2d(cnt, 7)
        cnt = cnt.view(cnt.size(0), -1)
        cnt = self.fc(cnt)
        ...
        # return output dict
        return {'count': torch.squeeze(cnt),
                'occupancy': torch.squeeze(occ),
                'heatmap': x}
